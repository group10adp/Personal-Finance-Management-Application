...
//            Toast.makeText(holder.itemView.getContext(), "Doc ID: " + docId, Toast.LENGTH_SHORT).show();
        ...
        holder.itemView.setOnLongClickListener(v -> {
            ...
            // Show a toast with the document ID
            showDeleteDialog(holder.itemView.getContext(), investment, position);
    ...
    private void showDeleteDialog(Context context, Investment investment, int position) {
        ...
        new AlertDialog.Builder(context)
                ...
                .setPositiveButton("Delete", (dialog, which) -> {
                    ...
                    deleteIncome(investment, position, context);
                })
    ...
    private void deleteIncome(Investment investment, int position, Context context) {
        String userId = FirebaseAuth.getInstance().getCurrentUser().getUid();  // Get the current user ID
        String date = investment.getDate();  // Assuming the `IncomeModel` contains a `getYear()` method
        String[] dateParts = date.split(" ");
        String year = dateParts[2];
        String month = getMonthNumber(dateParts[1]);


        String docId = investment.getDocId();  // The document ID of the income entry
        ...
        if (userId != null && year != null && month != null && docId != null) {
            ...
            documentReference.delete()
                    .addOnSuccessListener(aVoid -> {
                        investments.remove(position);  // Remove the item from the local list
                        notifyItemRemoved(position);  // Notify the adapter
                        Toast.makeText(context, "Investment deleted successfully", Toast.LENGTH_SHORT).show();
                    })
                    .addOnFailureListener(e -> {
                        Toast.makeText(context, "Error deleting investment", Toast.LENGTH_SHORT).show();
                    });

            DocumentReference documentReference2 = db.collection("users")
                    .document(userId)
                    .collection("expense")
                    .document(year)
                    .collection(month)
